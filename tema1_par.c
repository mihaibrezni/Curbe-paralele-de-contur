...
void rescale_image(void *params_data)
{
    uint8_t sample[3];
    ThreadParams params = *(ThreadParams *)params_data;

    int chunk_size = RESCALE_X / params.num_threads;

    int start_row = params.thread_id * chunk_size;
    int end_row = fmin ( ( ( params.thread_id + 1) * chunk_size ), RESCALE_X );

    // use bicubic interpolation for scaling
    for (int i = start_row; i < end_row; i++)
    {
        for (int j = 0; j < params.scaled_image->y; j++)
        {
            float u = (float)i / (float)(params.scaled_image->x - 1);
            float v = (float)j / (float)(params.scaled_image->y - 1);
            sample_bicubic(params.image, u, v, sample);

            params.scaled_image->data[i * params.scaled_image->y + j].red = sample[0];
            params.scaled_image->data[i * params.scaled_image->y + j].green = sample[1];
            params.scaled_image->data[i * params.scaled_image->y + j].blue = sample[2];
        }
    }
...
int main(int argc, char *argv[])
{
    if (argc < 3)
    {
        fprintf(stderr, "Usage: ./tema1 <in_file> <out_file> threads\n");
        return 1;
    }
    ...
    else
    {
        ...
        for (int i = 0; i < num_threads; i++)
        {
            ...
            thread_params[i].image = image;
            thread_params[i].scaled_image = scaled_image;
            thread_params[i].num_threads = num_threads;
            thread_params[i].thread_id = i;

            if (pthread_create(threads + i, NULL, rescale_image, thread_params + i))
            {
